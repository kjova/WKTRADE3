 ...
 ### calculate state for DRB_MOL
    ...
    for(j in 1:nrow(loopdata)){
      if (loopdata$Depl_DRB_MOL[j] > 0 ) {
        state[j] <- RBS_sens(Fd=loopdata$Depl_DRB_MOL[j],a=loopdata$slope[j],b=loopdata$intercept[j])
      }}
 ...
 ### calculate state for OT_CRU (combined)
    ...
    for(j in 1:nrow(loopdata)){
      if (loopdata$Depl_OT_CRU[j] > 0 ) {
        state[j] <- RBS_sens(Fd=loopdata$Depl_OT_CRU[j],a=loopdata$slope[j],b=loopdata$intercept[j])
      }}
 ...
 ### calculate state for OT_DMF
    ...
    for(j in 1:nrow(loopdata)){
      if (loopdata$Depl_OT_DMF[j] > 0 ) {
        state[j] <- RBS_sens(Fd=loopdata$Depl_OT_DMF[j],a=loopdata$slope[j],b=loopdata$intercept[j])
      }}
 ...
 ### calculate state for OT_MIX (combined)
    ...
    for(j in 1:nrow(loopdata)){
      if (loopdata$Depl_OT_MIX[j] > 0 ) {
        state[j] <- RBS_sens(Fd=loopdata$Depl_OT_MIX[j],a=loopdata$slope[j],b=loopdata$intercept[j])
      }}
 ...
 ### calculate state for OT_SPF
    ...
    for(j in 1:nrow(loopdata)){
      if (loopdata$Depl_OT_SPF[j] > 0 ) {
        state[j] <- RBS_sens(Fd=loopdata$Depl_OT_SPF[j],a=loopdata$slope[j],b=loopdata$intercept[j])
      }}
 ...
 ### calculate state for SDN_DMF
    ...
    for(j in 1:nrow(loopdata)){
      if (loopdata$Depl_SDN_DMF[j] > 0 ) {
        state[j] <- RBS_sens(Fd=loopdata$Depl_SDN_DMF[j],a=loopdata$slope[j],b=loopdata$intercept[j])
      }}
 ...
 ### calculate state for SSC_DMF
    ...
    for(j in 1:nrow(loopdata)){
      if (loopdata$Depl_SSC_DMF[j] > 0 ) {
        state[j] <- RBS_sens(Fd=loopdata$Depl_SSC_DMF[j],a=loopdata$slope[j],b=loopdata$intercept[j])
      }}
 ...
 ### calculate state for TBB_CRU
    ...
    for(j in 1:nrow(loopdata)){
      if (loopdata$Depl_TBB_CRU[j] > 0 ) {
        state[j] <- RBS_sens(Fd=loopdata$Depl_TBB_CRU[j],a=loopdata$slope[j],b=loopdata$intercept[j])
      }}
 ...
 ### calculate state for TBB_DMF
    ...
    for(j in 1:nrow(loopdata)){
      if (loopdata$Depl_TBB_DMF[j] > 0 ) {
        state[j] <- RBS_sens(Fd=loopdata$Depl_TBB_DMF[j],a=loopdata$slope[j],b=loopdata$intercept[j])
      }}
 ...
 ### calculate state for TBB_MOL
    ...
    for(j in 1:nrow(loopdata)){
      if (loopdata$Depl_TBB_MOL[j] > 0 ) {
        state[j] <- RBS_sens(Fd=loopdata$Depl_TBB_MOL[j],a=loopdata$slope[j],b=loopdata$intercept[j])
      }}
